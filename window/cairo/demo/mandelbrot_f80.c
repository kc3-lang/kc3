...
static cairo_surface_t *g_mandelbrot_f80_surface = NULL;

static bool mandelbrot_f80_resize (s_sequence *seq);
static bool mandelbrot_f80_update (s_sequence *seq);

bool mandelbrot_f80_button (s_sequence *seq, u8 button, s64 x, s64 y)
{
  s_map *map;
  f80 *next_x;
  f80 *next_y;
...
bool mandelbrot_f80_load (s_sequence *seq)
{
  s_map *map;
  assert(seq);
...
bool mandelbrot_f80_render (s_sequence *seq)
{
  cairo_t *cr;
  uw *h;
  s_map *map;
  ...
  cairo_set_source_surface(cr, g_mandelbrot_f80_surface, 0, 0);
...
static bool mandelbrot_f80_resize (s_sequence *seq)
{
  ...
  win = seq->window;
  assert(win);
  cairo_surface_destroy(g_mandelbrot_f80_surface);
  g_mandelbrot_f80_surface =
    cairo_image_surface_create(CAIRO_FORMAT_RGB24,
                               win->w, win->h);
  return true;
}

bool mandelbrot_f80_unload (s_sequence *seq)
{
  (void) seq;
  cairo_surface_destroy(g_mandelbrot_f80_surface);
  g_mandelbrot_f80_surface = NULL;
  return true;
}
...
static bool mandelbrot_f80_update (s_sequence *seq)
{
  ...
  assert(map->value[3].type == TAG_F80);
  next_z = map->value[3].data.td_f80;
  cairo_surface_flush(g_mandelbrot_f80_surface);
  data = cairo_image_surface_get_data(g_mandelbrot_f80_surface);
  assert(data);
  stride = cairo_image_surface_get_stride(g_mandelbrot_f80_surface);
  i = 0;
  while (i < win->h) {
