...
TEST_CASE(facts_with_)
{
  ...
  TEST_EQ(facts_with_cursor_next(&cursor, &f), &f);
  TEST_ASSERT(f);
  TAG_TEST_EQ(&subject.data.pvar->tag, tag);
  TAG_TEST_EQ(&predicate.data.pvar->tag, tag + 1);
  TAG_TEST_EQ(&object.data.pvar->tag, tag + 2);
  TEST_EQ(facts_with_cursor_next(&cursor, &f), &f);
  TEST_ASSERT(f);
  TAG_TEST_EQ(&subject.data.pvar->tag, tag);
  TAG_TEST_EQ(&predicate.data.pvar->tag, tag + 1);
  TAG_TEST_EQ(&object.data.pvar->tag, tag + 3);
  TEST_EQ(facts_with_cursor_next(&cursor, &f), &f);
  TEST_ASSERT(f);
  TAG_TEST_EQ(&subject.data.pvar->tag, tag);
  TAG_TEST_EQ(&predicate.data.pvar->tag, tag + 4);
  TAG_TEST_EQ(&object.data.pvar->tag, tag + 3);
  TEST_EQ(facts_with_cursor_next(&cursor, &f), &f);
  TEST_ASSERT(f);
  TAG_TEST_EQ(&subject.data.pvar->tag, tag + 5);
  TAG_TEST_EQ(&predicate.data.pvar->tag, tag + 1);
  TAG_TEST_EQ(&object.data.pvar->tag, tag + 2);
  TEST_EQ(facts_with_cursor_next(&cursor, &f), &f);
  TEST_ASSERT(! f);
