...
sw buf_parse_sw (s_buf *buf, sw *dest)
{
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_sw_base(buf, &g_kc3_base_binary,
                                    negative, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_sw_base(buf, &g_kc3_base_octal,
                                    negative, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  ...
  if (r > 0) {
    ...
    buf_save_clean(buf, &digits);
    if ((r1 = buf_parse_sw_base(buf, g_kc3_bases_hexadecimal + 1,
                                     negative, &tmp1)) < 0) {
      r = r1;
      goto restore;
    }
  ...
  if ((r = buf_parse_sw_base(buf, &g_kc3_base_decimal,
                                  negative, dest)) <= 0)
    goto restore;
  result += r;
 ok:
                           ...
                           bool negative, sw *dest)
{
  ...
  while ((r = buf_parse_digit(buf, base, &digit)) > 0) {
    ...
    u *= radix;
    if (negative ? (-((sw) u) < SW_MIN + digit) :
        (u > (uw) SW_MAX - digit)) {
      err_puts("buf_parse_sw_base: +: integer overflow");
      assert(! "buf_parse_sw_base: +: integer overflow");
      r = -1;
      goto restore;
    }
    u += digit;
