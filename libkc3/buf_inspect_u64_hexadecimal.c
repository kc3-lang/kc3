...
sw buf_inspect_u64_hexadecimal (s_buf *buf, u64 u)
{
  return buf_inspect_u64_base
    (buf, &g_kc3_base_hexadecimal, u);
}

u8 buf_inspect_u64_hexadecimal_digits (u64 x)
{
  return buf_inspect_u64_base_digits
                                   ...
                                   u64 x)
{
  ...
  assert(buf);
  assert(size);
  digits = buf_inspect_u64_hexadecimal_digits(x);
  i = digits;
  while (i < size) {
  ...
  if ((r = buf_inspect_u64_hexadecimal(buf, x)) <= 0)
    return r;
  result += r;
  assert(size == result);
  return result;
...
{
  return buf_inspect_u64_base_size(pretty, &g_kc3_base_hexadecimal,
                                       x);
}
