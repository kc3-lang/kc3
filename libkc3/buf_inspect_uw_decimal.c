...
{
  return buf_inspect_uw_base_digits(&g_kc3_base_decimal, x);
}
                                   ...
                                   const uw *x)
{
  ...
  assert(size);
  assert(x);
  digits = buf_inspect_uw_decimal_digits(x);
  i = digits;
  while (i < size) {
  ...
  if ((r = buf_inspect_uw_decimal(buf, x)) <= 0)
    return r;
  result += r;
  assert(size == result);
  return result;
...
{
  return buf_inspect_uw_base_size(pretty, &g_kc3_base_decimal,
                                       u);
}
