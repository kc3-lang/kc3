...
sw buf_parse_s32 (s_buf *buf, s32 *dest)
{
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_s32_base(buf, &g_kc3_base_binary,
                                    negative, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_s32_base(buf, &g_kc3_base_octal,
                                    negative, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  ...
  if (r > 0) {
    ...
    buf_save_clean(buf, &digits);
    if ((r1 = buf_parse_s32_base(buf, g_kc3_bases_hexadecimal + 1,
                                     negative, &tmp1)) < 0) {
      r = r1;
      goto restore;
    }
  ...
  if ((r = buf_parse_s32_base(buf, &g_kc3_base_decimal,
                                  negative, dest)) <= 0)
    goto restore;
  result += r;
 ok:
                           ...
                           bool negative, s32 *dest)
{
  ...
  while ((r = buf_parse_digit(buf, base, &digit)) > 0) {
    ...
    u *= radix;
    if (negative ? (-((s32) u) < S32_MIN + digit) :
        (u > (u32) S32_MAX - digit)) {
      err_puts("buf_parse_s32_base: +: integer overflow");
      assert(! "buf_parse_s32_base: +: integer overflow");
      r = -1;
      goto restore;
    }
    u += digit;
...
sw buf_parse_s32_decimal (s_buf *buf, bool negative,
                              s32 *dest)
{
  return buf_parse_s32_base(buf, &g_kc3_base_decimal,
                                negative, dest);
}
...
s_tag * buf_parse_tag_s32 (s_buf *buf, s_tag *dest)
{
  s_tag tmp = {0};
  tmp.type = TAG_S32;
  if (buf_parse_s32(buf, &tmp.data.td_s32) <= 0)
    return NULL;
  *dest = tmp;
  return dest;
}
