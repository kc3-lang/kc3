...
sw buf_inspect_u64_binary (s_buf *buf, u64 u)
{
                                   ...
                                   u64 x)
{
  ...
  assert(buf);
  assert(size);
  digits = buf_inspect_u64_binary_digits(x);
  i = digits;
  while (i < size) {
  ...
  if ((r = buf_inspect_u64_binary(buf, x)) <= 0)
    return r;
  result += r;
  assert(size == result);
  return result;
...
{
  return buf_inspect_u64_base_size(pretty, &g_kc3_base_binary,
                                       x);
}
