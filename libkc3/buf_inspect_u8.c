...
sw buf_inspect_u8 (s_buf *buf, u8 x)
{
  ...
  if ((r = buf_inspect_u8_base(buf, &g_kc3_base_decimal,
                                    x)) < 0)
    return r;
  result += r;
  return result;
...
sw buf_inspect_u8_base (s_buf *buf, const s_str *base, u8 x)
{
  character *c;
  u8 digit;
  sw i;
...
sw buf_inspect_u8_size (s_pretty *pretty, u8 x)
{
  sw r;
  sw result = 0;
  if (g_buf_inspect_type != &g_sym_U8 &&
      cast_needed_u8(x)) {
    if ((r = buf_inspect_paren_sym_size(pretty, &g_sym_U8)) < 0)
      return r;
    result += r;
    result += strlen(" ");
  }
  if ((r = buf_inspect_u8_base_size(pretty, &g_kc3_base_decimal,
                                         x)) < 0)
    return r;
  result += r;
  return result;
