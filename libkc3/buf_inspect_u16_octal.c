...
u8 buf_inspect_u16_octal_digits (const u16 *x)
{
  return buf_inspect_u16_base_digits(&g_kc3_base_octal, x);
}
                                   ...
                                   const u16 *x)
{
  ...
  assert(size);
  assert(x);
  digits = buf_inspect_u16_octal_digits(x);
  i = digits;
  while (i < size) {
  ...
  if ((r = buf_inspect_u16_octal(buf, x)) <= 0)
    return r;
  result += r;
  assert(size == result);
  return result;
