...
sw buf_parse_s16 (s_buf *buf, s16 *dest)
{
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_s16_base(buf, &g_kc3_base_binary,
                                    negative, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_s16_base(buf, &g_kc3_base_octal,
                                    negative, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  ...
  if (r > 0) {
    ...
    buf_save_clean(buf, &digits);
    if ((r1 = buf_parse_s16_base(buf, g_kc3_bases_hexadecimal + 1,
                                     negative, &tmp1)) < 0) {
      r = r1;
      goto restore;
    }
  ...
  if ((r = buf_parse_s16_base(buf, &g_kc3_base_decimal,
                                  negative, dest)) <= 0)
    goto restore;
  result += r;
 ok:
...
sw buf_parse_s16_decimal (s_buf *buf, bool negative,
                              s16 *dest)
{
  return buf_parse_s16_base(buf, &g_kc3_base_decimal,
                                negative, dest);
}
...
s_tag * buf_parse_tag_s16 (s_buf *buf, s_tag *dest)
{
  s_tag tmp = {0};
  tmp.type = TAG_S16;
  if (buf_parse_s16(buf, &tmp.data.td_s16) <= 0)
    return NULL;
  *dest = tmp;
  return dest;
}
