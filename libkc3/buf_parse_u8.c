...
sw buf_parse_u8 (s_buf *buf, u8 *dest)
{
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_u8_base(buf, &g_kc3_base_binary, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  }
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_u8_base(buf, &g_kc3_base_octal, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  }
  ...
  if (r > 0) {
    ...
    if ((r = buf_parse_u8_base(buf, g_kc3_bases_hexadecimal,
                                    &tmp)) < 0) {
    ...
    buf_save_clean(buf, &digits);
    if ((r1 = buf_parse_u8_base(buf, g_kc3_bases_hexadecimal + 1,
                                     &tmp1)) < 0) {
      r = r1;
      goto restore;
    }
  ...
  if ((r = buf_parse_u8_base(buf, &g_kc3_base_decimal, dest)) <= 0)
    goto restore;
  result += r;
 ok:
  r = result;
...
sw buf_parse_u8_decimal (s_buf *buf, u8 *dest)
{
  return buf_parse_u8_base(buf, &g_kc3_base_decimal, dest);
}
