...
u8 buf_inspect_u8_decimal_digits (const u8 *x)
{
  return buf_inspect_u8_base_digits(&g_kc3_base_decimal, x);
}
                                   ...
                                   const u8 *x)
{
  ...
  assert(size);
  assert(x);
  digits = buf_inspect_u8_decimal_digits(x);
  i = digits;
  while (i < size) {
  ...
  if ((r = buf_inspect_u8_decimal(buf, x)) <= 0)
    return r;
  result += r;
  assert(size == result);
  return result;
