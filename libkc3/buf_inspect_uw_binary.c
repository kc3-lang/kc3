...
u8 buf_inspect_uw_binary_digits (const uw *x)
{
  return buf_inspect_uw_base_digits(&g_kc3_base_binary, x);
}
                                   ...
                                   const uw *x)
{
  ...
  assert(size);
  assert(x);
  digits = buf_inspect_uw_binary_digits(x);
  i = digits;
  while (i < size) {
  ...
  if ((r = buf_inspect_uw_binary(buf, x)) <= 0)
    return r;
  result += r;
  assert(size == result);
  return result;
