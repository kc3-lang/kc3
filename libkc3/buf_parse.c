...
sw buf_parse_call_op_rec (s_buf *buf, s_call *dest, u8 min_precedence)
{
  ...
  next_op = next_op_tag.data.td_pstruct->data;
...
sw buf_parse_fn (s_buf *buf, s_fn *dest)
{
  ...
  while (l) {
    if (! frame_binding_new(tmp.frame,
                            l->tag.data.td_ptuple->tag[0].data.td_psym,
                            l->tag.data.td_ptuple->tag + 1))
      goto restore;
    l = list_next(l);
  }
...
sw buf_parse_integer_unsigned_bin (s_buf *buf, s_integer *dest)
{
  ...
  if (mp_add_d(&dest->in_int, digit, &dest->in_int) != MP_OKAY)
    goto error;
  while ((r = buf_read_1(buf, "_")) >= 0 &&
         (r = buf_parse_digit_bin(buf, &digit)) > 0) {
    result += r;
    if (mp_mul_d(&dest->in_int, radix, &dest->in_int) != MP_OKAY ||
        mp_add_d(&dest->in_int, digit, &dest->in_int) != MP_OKAY)
      goto error;
  }
  if (result > 0)
    r = result;
...
sw buf_parse_integer_unsigned_dec (s_buf *buf, s_integer *dest)
{
  ...
  while ((r = buf_read_1(buf, "_")) >= 0 &&
    ...
    result += r;
    if (mp_mul_d(&tmp.in_int, radix, &tmp2.in_int) != MP_OKAY ||
        mp_add_d(&tmp2.in_int, digit, &tmp.in_int) != MP_OKAY)
      goto error;
    integer_clean(&tmp2);
  }
  *dest = tmp;
...
sw buf_parse_list_tag (s_buf *buf, s_tag *dest)
{
  ...
  if (r > 0) {
    ...
    key.type = TAG_PSYM;
    key.data.td_psym = str_to_sym(&str);
    str_clean(&str);
    tmp.data.td_ptuple->tag[0] = key;
    tmp.data.td_ptuple->tag[1] = value;
    goto ok;
  }
 tag:
  result = 0;
  ...
  while (r == 0) {
    *keys_end = list_new(NULL);
    if ((r = buf_parse_map_key(buf, &(*keys_end)->tag)) <= 0)
      goto restore;
    result += r;
    keys_end = &(*keys_end)->next.data.td_plist;
    if ((r = buf_parse_comments(buf)) < 0)
      goto restore;
    result += r;
    if ((r = buf_ignore_spaces(buf)) < 0)
    ...
    *values_end = list_new(NULL);
    if ((r = buf_parse_tag(buf, &(*values_end)->tag)) <= 0)
      goto restore;
    result += r;
    values_end = &(*values_end)->next.data.td_plist;
    if ((r = buf_parse_comments(buf)) < 0)
      goto restore;
    result += r;
    if ((r = buf_ignore_spaces(buf)) < 0)
...
sw buf_parse_map_key_str (s_buf *buf, s_tag *dest)
{
  ...
  if (r > 0) {
    ...
    if (r > 0) {
      result += r;
      dest->type = TAG_PSYM;
      dest->data.td_psym = str_to_sym(&str);
      str_clean(&str);
      goto ok;
    }
    if ((r = buf_parse_comments(buf)) < 0)
...
sw buf_parse_match (s_buf *buf, p_call *pcall)
{
  ...
  while (1) {
    if (! (*keys_tail = list_new(NULL)) ||
        ...
        ! (do_block = &(*values_tail)->tag.data.td_do_block) ||
    ...
    if (buf->rpos > 3 &&
        buf->ptr.p_pchar[buf->rpos - 3] == 'e' &&
        buf->ptr.p_pchar[buf->rpos - 2] == 'n' &&
        buf->ptr.p_pchar[buf->rpos - 1] == 'd')
      break;
  }
...
sw buf_parse_module_name (s_buf *buf, p_sym *dest)
{
  ...
  memcpy(str.free.p_pvoid, buf->ptr.p_pchar + save.rpos, str.size);
  tmp = str_to_sym(&str);
  str_clean(&str);
  if (! tmp) {
    r = -1;
...
sw buf_parse_pcall_access (s_buf *buf, p_call *dest)
{
  ...
  list_next(tmp.arguments)->tag.data.td_plist = key;
  if (! (*dest = alloc(sizeof(s_call)))) {
    r = -1;
    goto restore;
  }
  ...
  while (1) {
    ...
    *addr_last = list_new(NULL);
    if ((r = buf_parse_tag(buf, &(*addr_last)->tag)) <= 0)
      goto restore;
    result += r;
    addr_last = &(*addr_last)->next.data.td_plist;
    if ((r = buf_ignore_spaces(buf)) < 0)
      goto restore;
    result += r;
    if ((r = buf_read_1(buf, "]")) <= 0)
...
sw buf_parse_pcall_if (s_buf *buf, p_call *dest)
{
  ...
  *args_last = list_new(NULL);
  ...
  if ((r = buf_ignore_spaces(buf)) < 0)
    goto restore;
  result += r;
  args_last = &(*args_last)->next.data.td_plist;
  *args_last = list_new(NULL);
  then = &(*args_last)->tag;
  ...
  *args_last = list_new(NULL);
  ...
  if (has_else) {
    ...
    else_->data.td_do_block = (s_do_block) {0};
...
sw buf_parse_static_tag (s_buf *buf, s_tag *tag)
{
  ...
  if (tmp.type == TAG_PCALL &&
      (! tmp.data.td_pcall->ident.module ||
       tmp.data.td_pcall->ident.module == &g_sym_KC3) &&
      tmp.data.td_pcall->ident.sym == &g_sym_str &&
      tmp.data.td_pcall->arguments &&
      tmp.data.td_pcall->arguments->tag.type == TAG_PLIST) {
    tag_void(&tmp);
    buf_save_restore_rpos(buf, &save);
    if ((r = buf_parse_str(buf, &str)) <= 0)
      goto clean;
...
sw buf_parse_tag_array (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_array(buf, &dest->data.td_array)) > 0)
    dest->type = TAG_ARRAY;
  return r;
}
...
sw buf_parse_tag_do_block (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_do_block(buf, &dest->data.td_do_block)) > 0)
    dest->type = TAG_DO_BLOCK;
  return r;
}
...
sw buf_parse_tag_bool (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_bool(buf, &dest->data.td_bool_)) > 0)
    dest->type = TAG_BOOL;
  return r;
}
...
sw buf_parse_tag_character (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_character(buf, &dest->data.td_character)) > 0)
...
sw buf_parse_tag_complex (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcomplex(buf, &dest->data.td_pcomplex)) > 0)
...
sw buf_parse_tag_cow (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcow(buf, &dest->data.td_pcow)) > 0)
    dest->type = TAG_PCOW;
  return r;
}
...
sw buf_parse_tag_f32 (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_f32(buf, &dest->data.td_f32)) > 0)
    dest->type = TAG_F32;
  return r;
}
...
sw buf_parse_tag_f64 (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_f64(buf, &dest->data.td_f64)) > 0)
    dest->type = TAG_F64;
  return r;
}
...
sw buf_parse_tag_f80 (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_f80(buf, &dest->data.td_f80)) > 0)
    dest->type = TAG_F80;
  return r;
}
...
sw buf_parse_tag_f128 (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_f128(buf, &dest->data.td_f128)) > 0)
    dest->type = TAG_F128;
  return r;
}
...
sw buf_parse_tag_ident (s_buf *buf, s_tag *dest)
{
  ...
  assert(dest);
  env = env_global();
  r = buf_parse_ident_cast(buf, &dest->data.td_ident);
  if (r > 0) {
    if (! dest->data.td_ident.module &&
        (tag = frame_get_w(env->read_time_frame,
                           dest->data.td_ident.sym)))
...
sw buf_parse_tag_ident_sym (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_ident_sym(buf, &dest->data.td_psym)) > 0)
    dest->type = TAG_PSYM;
  return r;
}
...
sw buf_parse_tag_integer (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_integer(buf, &dest->data.td_integer)) > 0)
...
sw buf_parse_tag_map (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_map(buf, &dest->data.td_map)) > 0)
    dest->type = TAG_MAP;
  return r;
}
...
sw buf_parse_tag_match (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_match(buf, &dest->data.td_pcall)) > 0)
    dest->type = TAG_PCALL;
  return r;
}
...
sw buf_parse_tag_module_name (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_module_name(buf, &dest->data.td_psym)) > 0)
    dest->type = TAG_PSYM;
  return r;
}
...
sw buf_parse_tag_pcall (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcall(buf, &dest->data.td_pcall)) > 0)
    dest->type = TAG_PCALL;
  return r;
}
...
sw buf_parse_tag_pcall_access (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcall_access(buf, &dest->data.td_pcall)) > 0)
    dest->type = TAG_PCALL;
  return r;
}
...
sw buf_parse_tag_pcall_brackets (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcall_brackets(buf, &dest->data.td_pcall)) > 0)
    dest->type = TAG_PCALL;
  return r;
}
...
sw buf_parse_tag_pcall_cast (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcall_cast(buf, &dest->data.td_pcall)) > 0)
    dest->type = TAG_PCALL;
  return r;
}
...
sw buf_parse_tag_pcall_if (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcall_if(buf, &dest->data.td_pcall)) > 0)
    dest->type = TAG_PCALL;
  return r;
}
...
sw buf_parse_tag_pcall_op (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcall_op(buf, &dest->data.td_pcall)) > 0)
    dest->type = TAG_PCALL;
  return r;
}
...
sw buf_parse_tag_pcall_paren (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcall_paren(buf, &dest->data.td_pcall)) > 0)
    dest->type = TAG_PCALL;
  return r;
}
...
sw buf_parse_tag_pcall_op_unary (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcall_op_unary(buf, &dest->data.td_pcall)) > 0)
    dest->type = TAG_PCALL;
  return r;
}
...
sw buf_parse_tag_pcallable (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pcallable(buf, &dest->data.td_pcallable)) > 0)
...
sw buf_parse_tag_plist (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_plist(buf, &dest->data.td_plist)) > 0)
    dest->type = TAG_PLIST;
  return r;
}
...
sw buf_parse_tag_pointer (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pointer(buf, &dest->data.td_pointer)) > 0)
...
sw buf_parse_tag_ptr (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_ptr(buf, &dest->data.td_ptr)) > 0)
    dest->type = TAG_PTR;
  return r;
}
...
sw buf_parse_tag_ptr_free (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_ptr_free(buf, &dest->data.td_ptr_free)) > 0)
...
sw buf_parse_tag_pvar (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_pvar(buf, &dest->data.td_pvar)) > 0)
    dest->type = TAG_PVAR;
  return r;
}
...
sw buf_parse_tag_quote (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_quote(buf, &dest->data.td_quote)) > 0)
    dest->type = TAG_QUOTE;
  return r;
}
...
sw buf_parse_tag_ratio (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_ratio(buf, &dest->data.td_ratio)) > 0)
    dest->type = TAG_RATIO;
  return r;
}
...
sw buf_parse_tag_special_operator (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_special_operator(buf, &dest->data.td_pcall)) > 0)
    dest->type = TAG_PCALL;
  return r;
}
...
sw buf_parse_tag_sym (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_sym(buf, &dest->data.td_psym)) > 0)
    dest->type = TAG_PSYM;
  return r;
}
...
sw buf_parse_tag_time (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_time(buf, &dest->data.td_time)) > 0)
    dest->type = TAG_TIME;
  return r;
}
...
sw buf_parse_tag_tuple (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_ptuple(buf, &dest->data.td_ptuple)) > 0)
...
sw buf_parse_tag_unquote (s_buf *buf, s_tag *dest)
{
  ...
  assert(buf);
  assert(dest);
  if ((r = buf_parse_unquote(buf, &dest->data.td_unquote)) > 0)
