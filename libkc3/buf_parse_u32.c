...
sw buf_parse_u32 (s_buf *buf, u32 *dest)
{
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_u32_base(buf, &g_kc3_base_binary, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  }
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_u32_base(buf, &g_kc3_base_octal, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  }
  ...
  if (r > 0) {
    ...
    if ((r = buf_parse_u32_base(buf, g_kc3_bases_hexadecimal,
                                    &tmp)) < 0) {
    ...
    buf_save_clean(buf, &digits);
    if ((r1 = buf_parse_u32_base(buf, g_kc3_bases_hexadecimal + 1,
                                     &tmp1)) < 0) {
      r = r1;
      goto restore;
    }
  ...
  if ((r = buf_parse_u32_base(buf, &g_kc3_base_decimal, dest)) <= 0)
    goto restore;
  result += r;
 ok:
  r = result;
...
s_tag * buf_parse_tag_u32 (s_buf *buf, s_tag *dest)
{
  s_tag tmp = {0};
  tmp.type = TAG_U32;
  if (buf_parse_u32(buf, &tmp.data.td_u32) <= 0)
    return NULL;
  *dest = tmp;
  return dest;
}
                                    ...
                                    s_tag *dest)
{
  s_tag tmp = {0};
  tmp.type = TAG_U32;
  if (buf_parse_u32_base(buf, base, &tmp.data.td_u32) <= 0)
    return NULL;
  *dest = tmp;
  return dest;
}
