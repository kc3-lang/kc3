...
(s8 *s, const s_sym * const *type, const s_tag *tag)
{
  ...
  case TAG_BOOL:
    *s = tag->data.td_bool_ ? 1 : 0;
    return s;
  case TAG_CHARACTER:
    *s = (s8) tag->data.td_character;
    return s;
  case TAG_F32:
    *s = (s8) tag->data.td_f32;
    return s;
  case TAG_F64:
    *s = (s8) tag->data.td_f64;
    return s;
  case TAG_INTEGER:
    *s = integer_to_s8(&tag->data.td_integer);
    return s;
  case TAG_RATIO:
    *s = ratio_to_s8(&tag->data.td_ratio);
    return s;
  case TAG_SW:
    *s = (s8) tag->data.td_sw;
    return s;
  case TAG_S64:
    *s = (s8) tag->data.td_s64;
    return s;
  case TAG_S32:
    *s = (s8) tag->data.td_s32;
    return s;
  case TAG_S16:
    *s = (s8) tag->data.td_s16;
    return s;
  case TAG_S8:
    *s = (s8) tag->data.td_s8;
    return s;
  case TAG_STR:
    return s8_init_str(s, &tag->data.td_str);
  case TAG_U8:
    *s = (s8) tag->data.td_u8;
    return s;
  case TAG_U16:
    *s = (s8) tag->data.td_u16;
    return s;
  case TAG_U32:
    *s = (s8) tag->data.td_u32;
    return s;
  case TAG_U64:
    *s = (s8) tag->data.td_u64;
    return s;
  case TAG_UW:
    *s = (s8) tag->data.td_uw;
    return s;
