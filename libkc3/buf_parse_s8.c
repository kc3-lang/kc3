...
sw buf_parse_s8 (s_buf *buf, s8 *dest)
{
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_s8_base(buf, &g_kc3_base_binary,
                                    negative, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  ...
  if (r > 0) {
    result += r;
    if ((r = buf_parse_s8_base(buf, &g_kc3_base_octal,
                                    negative, dest)) <= 0)
      goto restore;
    result += r;
    goto ok;
  ...
  if (r > 0) {
    ...
    buf_save_clean(buf, &digits);
    if ((r1 = buf_parse_s8_base(buf, g_kc3_bases_hexadecimal + 1,
                                     negative, &tmp1)) < 0) {
      r = r1;
      goto restore;
    }
  ...
  if ((r = buf_parse_s8_base(buf, &g_kc3_base_decimal,
                                  negative, dest)) <= 0)
    goto restore;
  result += r;
 ok:
                           ...
                           bool negative, s8 *dest)
{
  ...
  while ((r = buf_parse_digit(buf, base, &digit)) > 0) {
    ...
    u *= radix;
    if (negative ? (-((s8) u) < S8_MIN + digit) :
        (u > (u8) S8_MAX - digit)) {
      err_puts("buf_parse_s8_base: +: integer overflow");
      assert(! "buf_parse_s8_base: +: integer overflow");
      r = -1;
      goto restore;
    }
    u += digit;
...
sw buf_parse_s8_decimal (s_buf *buf, bool negative,
                              s8 *dest)
{
  return buf_parse_s8_base(buf, &g_kc3_base_decimal,
                                negative, dest);
}
