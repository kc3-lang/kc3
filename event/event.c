...
#endif
{
  ...
  if (tag->data.td_ptuple->tag[0].type != TAG_PCALLABLE) {
  ...
  if (! (callable = tag->data.td_ptuple->tag[0].data.td_pcallable)) {
  ...
  if (tag->data.td_ptuple->tag[1].type != TAG_PTR) {
    err_puts("kc3_event_callback: invalid arg 2: not a Ptr");
    assert(! "kc3_event_callback: invalid arg 2: not a Ptr");
    abort();
  }
  ev = tag->data.td_ptuple->tag[1].data.td_ptr.p_pvoid;
  arg = tag->data.td_ptuple->tag + 2;
  events_list = NULL;
                              ...
                              p_callable *callback, s_tag *arg)
{
  ...
  while (e) {
    ...
    if (e->tag.data.td_psym == &g_sym_read)
      events_s16 |= EV_READ;
    else if (e->tag.data.td_psym == &g_sym_write)
      events_s16 |= EV_WRITE;
    else if (e->tag.data.td_psym == &g_sym_persist)
      events_s16 |= EV_PERSIST;
    else if (e->tag.data.td_psym == &g_sym_timeout)
  ...
  tag_init_pcallable_copy(tag->data.td_ptuple->tag, callback);
  tag_init_copy(tag->data.td_ptuple->tag + 2, arg);
  tag_init_ptr(tag->data.td_ptuple->tag + 1, ev);
  return ev;
 invalid_event_list:
