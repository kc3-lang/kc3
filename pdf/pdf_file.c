                                      ...
                                      s_tag *ref, s_tag *dest)
{
  ...
  if (ref->type != TAG_PTUPLE ||
      ...
      ref->data.td_ptuple->tag[0].data.td_psym != sym_1("indirect_object") ||
      ref->data.td_ptuple->tag[1].type != TAG_U32 ||
      ref->data.td_ptuple->tag[2].type != TAG_U16) {
  ...
  object_number = ref->data.td_ptuple->tag[1].data.td_u32;
  generation_number = ref->data.td_ptuple->tag[2].data.td_u16;
  i = 0;
  while (i < pdf_file->body.count) {
    if (pdf_file->body.key[i].type != TAG_PTUPLE ||
        pdf_file->body.key[i].data.td_ptuple->count != 2 ||
        pdf_file->body.key[i].data.td_ptuple->tag[0].type != TAG_U32 ||
        pdf_file->body.key[i].data.td_ptuple->tag[1].type != TAG_U16) {
      err_puts("pdf_file_get_indirect_object: invalid pdf file body");
      assert(! "pdf_file_get_indirect_object: invalid pdf file body");
      return NULL;
    }
    if (pdf_file->body.key[i].data.td_ptuple->tag[0].data.td_u32 ==
        object_number &&
        pdf_file->body.key[i].data.td_ptuple->tag[1].data.td_u16 ==
        generation_number)
