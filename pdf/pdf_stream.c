                             ...
                             s_array *color, bool sb)
{
  ...
  if ((r = pdf_buf_write_flat_array(stream->buf, color)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf,
                                     sb ? " K" : " k", true)) < 0)
    return r;
  result += r;
  return result;
...
sw pdf_stream_color_use_shading (s_pdf_stream *stream, p_sym resource)
{
  ...
  if ((r = pdf_buf_write_name(stream->buf, resource)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " sh",
                                              true)) < 0)
    return r;
  result += r;
  return result;
                         ...
                         enum pdf_fill_mode fill)
{
  ...
  assert(stream);
  assert(stream->buf);
    if ((r = pdf_buf_write_token_with_separator(stream->buf, "S",
                                                true)) < 0)
      return r;
    result += r;
  }
                               ...
                               enum pdf_fill_mode fill)
{
  ...
  if (stroke) {
    if ((r = pdf_buf_write_token_with_separator(stream->buf, "b",
                                                true)) < 0)
      return r;
    result += r;
  }
...
sw pdf_stream_draw_path_begin_at(s_pdf_stream *stream, s_vec2 *pos)
{
  ...
  if ((r = pdf_buf_write_vec2(stream->buf, pos)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " m",
                                              true)) < 0)
    return r;
  result += r;
  return result;
                                       ...
                                       s_vec2 *end)
{
  ...
  if ((r = pdf_buf_write_vec2(stream->buf, end)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " c",
                                              true)) < 0)
    return r;
  result += r;
  return result;
                                         ...
                                         s_vec2 *end)
{
  ...
  if ((r = pdf_buf_write_vec2(stream->buf, end)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " v",
                                              true)) < 0)
    return r;
  result += r;
  return result;
                                         ...
                                         s_vec2 *end)
{
  ...
  if ((r = pdf_buf_write_vec2(stream->buf, end)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " y",
                                              true)) < 0)
    return r;
  result += r;
  return result;
...
sw pdf_stream_draw_path_move_to(s_pdf_stream *stream, s_vec2 *pos)
{
  ...
  if ((r = pdf_buf_write_vec2(stream->buf, pos)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " l",
                                              true)) < 0)
    return r;
  result += r;
  return result;
...
sw pdf_stream_draw_path_rect(s_pdf_stream *stream, s_pdf_rect *rect)
{
  ...
  if ((r = pdf_buf_write_float(stream->buf, rect->h)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " re",
                                              true)) < 0)
    return r;
  result += r;
  return result;
                                ...
                                enum pdf_line_cap cap)
{
  ...
  if ((r = pdf_buf_write_integer(stream->buf, (s32)cap)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " J",
                                              true)) < 0)
    return r;
  result += r;
  return result;
                                         ...
                                         s32 phase)
{
  ...
  if ((r = pdf_buf_write_integer(stream->buf, phase)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " d",
                                              true)) < 0)
    return r;
  result += r;
  return result;
...
sw pdf_stream_draw_set_line_width(s_pdf_stream *stream, f32 width)
{
  ...
  if ((r = pdf_buf_write_float(stream->buf, width)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " w",
                                              true)) < 0)
    return r;
  result += r;
  return result;
                                             ...
                                             s_array *m)
{
  ...
  if ((r = pdf_buf_write_flat_array(stream->buf, m)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " cm",
                                              true)) < 0)
    return r;
  result += r;
  return result;
...
sw pdf_stream_external_object(s_pdf_stream *stream, p_sym name)
{
  ...
  if ((r = pdf_buf_write_name(stream->buf, name)) < 0)
    return r;
  result += r;
  if ((r = pdf_buf_write_token_with_separator(stream->buf, " Do",
                                              true)) < 0)
    return r;
  result += r;
  return result;
...
sw pdf_stream_text_begin (s_pdf_stream *stream)
{
  assert(stream);
  assert(stream->buf);
  return pdf_buf_write_token_with_separator(stream->buf, "BT", true);
}
                             ...
                             p_sym font, s32 size)
{
  ...
  result += r;
  return result + pdf_buf_write_token_with_separator(stream->buf, "Tf",
                                                     true);
}
...
sw pdf_stream_text_set_pos_xy (s_pdf_stream *stream, s_vec2 *pos)
{
  ...
  result += r;
  return result + pdf_buf_write_token_with_separator(stream->buf, "Td",
                                                     true);
}
...
sw pdf_stream_text_set_text (s_pdf_stream *stream, s_str *text)
{
  ...
  return r + pdf_buf_write_token_with_separator(stream->buf, "Tj",
                                                true);
}
